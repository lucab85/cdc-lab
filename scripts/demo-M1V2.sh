...
echo -e "\n${YELLOW}Run query:${NC}"
echo "SELECT order_id, status, order_total FROM orders_iceberg WHERE order_id = 101;"
echo ""
echo -e "${YELLOW}Or query the source PostgreSQL directly:${NC}"
echo "  docker exec -it cdc-lab-trino-1 trino --catalog postgres"
echo "  SELECT * FROM public.orders WHERE order_id = 101;"
